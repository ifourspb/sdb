<?php
/**
* WordPress bytehand class
*
* Extends the bytehand wrapper class to use the
* WordPress HTTP API for HTTP calls, attempts to work
* round the differences in PHP versions, such as SSL
* & curl support
*
* @package     bytehand
* @subpackage  WordPressbytehand       
* @since       1.0
*/

class WordPressBytehand extends bytehand {

  /**
   * Options key for bytehand plugins
   */
  const OPTIONS_KEY = 'bytehand_options';

  /**
   * SSL options key for bytehand plugins
   */
  const SSL_OPTIONS_KEY = 'bytehand_ssl_options';
   ...
   * used in WordPressbytehand for the purposes of converting to an API key.
   *
   * @param   string  key         Your bytehand API Key
   * @param   array   options     Optional parameters for sending SMS
   */
  public function __construct( $arg1, $arg2, $arg3 = array() ) {
  ...
  */
  protected function xmlPost( $url, $data ) {
    ...
    if( is_wp_error( $result ) ) {
      ...
      throw new bytehandException( "HTTP Call failed - Error: " . $result->get_error_message() );
    }
    return $result[ 'body' ];
  }
