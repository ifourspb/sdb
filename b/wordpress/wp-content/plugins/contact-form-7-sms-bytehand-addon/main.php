...
class bytehand_CF7_Plugin extends bytehand_Plugin {  

  protected $plugin_name = 'Contact Form 7 SMS Plugin (ByteHand)';  
  protected $language_string = 'wpcf7_sms_bh';
  protected $prefix = 'bytehand_cf7';
  protected $folder = '';
  ...
  public function __construct() {
    ...
    // Set the plugin's bytehand SMS menu to load the contact forms
  ...
  /**
   * Setup the admin navigation, etc
   *
   * @return void
   */
  ...
  /**
   * Setup HTML for the admin <head> tag
   *
   * @return void
   */
  ...
  public function send_sms( $form ) {
	...
	$formOptions = get_option( 'wpcf7_sms_' . (method_exists($form, 'id') ? $form->id() : $form->id));
	if ($formOptions) {
